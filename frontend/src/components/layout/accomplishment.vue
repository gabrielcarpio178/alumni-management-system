<template>
    <div class="w-full">
        <div class="flex justify-end items-center gap-x-3 py-1">
            <h2>Batch By: </h2>
            <select v-model="year" @change="getYear" class="w-1/4 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="all" selected>
                    All
                </option>
                <option :value="year_content" v-for="(year_content, index) in years_content" :key="index">
                    {{year_content}}
                </option>
            </select>
        </div>
        <div v-if="datas.length===0">
            <h1>No Alumni in this batch</h1>
        </div>
...
export default {
    methods:{
        getYear(){
            this.filterData(this.year);
        }
    },
    props: ['years_content', 'datas', 'filterData', 'waitToResult']
}
</script>